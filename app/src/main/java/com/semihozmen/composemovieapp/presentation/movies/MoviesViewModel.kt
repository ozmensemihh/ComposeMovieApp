...
import com.semihozmen.composemovieapp.domain.use_case.get_movies.GetMoviesUseCase
...
class MoviesViewModel @Inject constructor(private val getMoviesUseCase: GetMoviesUseCase) : ViewModel() {
    ...
    fun getMovies(search :String){
        ...
        job = getMoviesUseCase.executeGetMovie(search).onEach {
            when(it){
                is Resource.Success ->{_state.value = MoviesState(movies = it.data?: emptyList())}
                is Resource.Error ->{_state.value = MoviesState(error=it.message?:"Error !")}
    ...
    fun onEvent (event: MoviesEvent){
        when(event){
            is MoviesEvent.Search ->{ getMovies(event.search) }
        }
    }
